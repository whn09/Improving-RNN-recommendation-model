...
def update_manager_command_parser(parser):
    parser.add_argument('--u_m', dest='update_manager',
                        choices=['sgd', 'adagrad', 'adadelta', 'rmsprop', 'nesterov', 'adam'], help='Update mechanism',
                        default='adagrad')
...
def get_update_manager(args):
    if args.update_manager == 'sgd':
        return SGD(learning_rate=args.u_l)
    if args.update_manager == 'adagrad':
        return Adagrad(learning_rate=args.u_l)  # gives nan
    elif args.update_manager == 'adadelta':
        return Adadelta()  # too slow
    elif args.update_manager == 'rmsprop':
        return RMSProp(learning_rate=args.u_l)
...
class SGD(object):
    def __init__(self, learning_rate=0.01, **kwargs):
        ...
        self.name = 'Us_lr' + str(self.learning_rate)

    def __call__(self, cost, params):
        return lasagne.updates.sgd(cost, params, self.learning_rate)
